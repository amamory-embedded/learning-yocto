...
Run the full image build again:
...
After building the image for qemu, we ar goiing to see two different images in the following directory:
...
After building again the image (this time it`s quick) you will find the following file:
...
Remember that the `/mnt/yocto/tmp` is shared between the docker image and the host, so it`s easy to burn the image file intos a SD card.
...
## Contributions
  ...
  Did you find a bug in this tutorial ? Do you have some extensions or updates to add ? Please me send a Pull Request.
